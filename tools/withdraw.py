...
"""Take a word out of rotation and put it back in the bank.
    ...
    python3 tools/withdraw.py --notes czesto,zwykle --why "interference"
    python3 tools/withdraw.py --notes czesto,zwykle --why "..." --go
    python3 tools/withdraw.py --list
    python3 tools/withdraw.py --restore czesto --go
...
APP = ROOT / "index.html"
BASE = ("https://firestore.googleapis.com/v1/projects/flashcards-f5b40/"
        "databases/(default)/documents/progress/")
...
def web_key():
    m = re.search(r'apiKey: "([^"]+)"', APP.read_text(encoding="utf-8"))
    if not m:
